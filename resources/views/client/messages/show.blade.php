...
<div class="flex items-center gap-4 mb-6">
    ...
    <div>
        <h1 class="text-2xl font-black text-white">{{ $conversation->subject }}</h1>
        <p class="text-sm text-white/50 mt-1">Conversation thread</p>
    </div>
</div>
...
<div class="bg-surface-800/60 backdrop-blur-sm rounded-2xl border border-white/6 flex flex-col" style="height: calc(100vh - 240px);">
    <div class="flex-1 overflow-y-auto p-6 space-y-4" id="messages-container">
        ...
        <div class="flex {{ $message->user_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
            <div class="max-w-md {{ $message->user_id === auth()->id() ? 'bg-primary-600 text-white' : 'bg-white/6 text-white/90' }} rounded-2xl p-4">
    ...
    <div class="border-t border-white/6 p-4">
        <form action="{{ route('client.messages.store', $conversation) }}" method="POST" enctype="multipart/form-data" class="flex gap-2">
            @csrf
