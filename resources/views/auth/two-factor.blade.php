...
<div class="min-h-screen flex items-center justify-center px-6 py-12">
    <div class="w-full max-w-[420px]">
        ...
        {{-- Card --}}
        <div class="relative">
            <div class="absolute -inset-px bg-linear-to-br from-emerald-500/15 via-transparent to-primary-500/15 rounded-3xl blur-sm"></div>
            <div class="relative bg-surface-900/70 backdrop-blur-2xl border border-white/[0.08] rounded-3xl p-8 sm:p-10 shadow-2xl shadow-black/30">
                ...
                <div class="flex justify-center mb-6">
                    <div class="relative">
                        <div class="absolute inset-0 bg-emerald-500/15 rounded-2xl blur-xl scale-150"></div>
                        <div class="relative w-16 h-16 bg-linear-to-br from-emerald-500/15 to-emerald-600/10 border border-emerald-500/20 text-emerald-400 rounded-2xl flex items-center justify-center">
                ...
                <div class="bg-red-500/10 border border-red-500/20 text-red-300 p-4 rounded-xl mb-6 text-sm flex items-start gap-3">
                    ...
                    <div class="pt-1">@foreach($errors->all() as $error)<p>{{ $error }}</p>@endforeach</div>
                </div>
                @endif
                ...
                <form method="POST" action="{{ route('2fa.verify') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="relative group">
                            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-emerald-400 transition-colors duration-300"><i class="fas fa-fingerprint text-sm"></i></span>
                            <input type="text" name="code" id="code" class="input-field pl-11 text-center text-xl tracking-[0.4em] font-mono" maxlength="10" placeholder="000000" autofocus autocomplete="one-time-code" required>
                        </div>
                        <p class="text-xs text-white/20 mt-2.5 text-center">Enter your 6-digit TOTP code or a recovery code</p>
                    </div>

                    <button type="submit" class="w-full relative group">
                        <div class="absolute -inset-0.5 bg-linear-to-r from-emerald-500 to-primary-500 rounded-xl blur opacity-50 group-hover:opacity-80 transition-opacity duration-500"></div>
                        <div class="relative bg-linear-to-r from-emerald-600 to-emerald-500 text-white py-3.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-emerald-500/25 transition-all duration-300">
                ...
                <div class="text-center mt-8 pt-6 border-t border-white/[0.06]">
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        ...
                        <button type="submit" class="text-sm text-white/30 hover:text-white/55 transition-colors inline-flex items-center gap-2 group">
                            <i class="fas fa-arrow-right-from-bracket text-xs group-hover:translate-x-0.5 transition-transform duration-300"></i> Sign out instead
                        </button>
                    </form>
                </div>
            </div>
